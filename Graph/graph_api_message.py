...
####----------------------------------------------------------
df2 = pd.read_excel('./Graph/b.xlsx',  thousands = ',')
...
for idx, row in df2.iterrows():
    ...
    if row['점포명'] not in sendMessage[row['OFC']]:
        sendMessage[row['OFC']][row['점포명']] = f'[{row["점포명"]}]점 스내킹 안내 드립니다..<br>'
    ...
    sendMessage[row['OFC']][row['점포명']] += f"달성여부: {c}<br>목표 {format(row['목표'],',')}<br>일발주수량:{format(row['일발주수량'],',')}<br>4월폐기:{format(row['4월_폐기'],',')}<br>초과지원금액:{format(int(row['초과분']),',')}"
print(sendMessage)
