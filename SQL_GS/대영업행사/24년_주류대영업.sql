...
join LGMJVDP.TB_STORE_OP_DM pa13
 ...
 -- 양수점포 제외하기 
 and pa13.OPEN_DT is not null and pa13.OPEN_DT < ('2024-06-01')  
join LGMJVDP.TB_STORE_OP_DM pa14 
 on (pa11.STORECD =  pa14.STORECD)
 -- 오픈일 기준 제외점 정하기 
and pa14.OPEN_DT < ('2024-05-11')  
...
inner join LGMJVDP.TB_STORE_OP_DM  pa14
...
with     gopa1 as
 (select    a11.STORECD  STORECD,
        a11.GOODCD GOODCD,
        sum(case when a11.GOODCD in ('8809718540141', '8809718540165', '8809741171831', '8809741171893', '8809741171817', '8809741171978', '8809741171442', '8809741171466', '8809741171619', '8809741171329', '8809741171596', '8809706306193', '8809706306209') then a11.ODR_QTY else 0 end)
        sum(case when a11.GOODCD in ('8809793600341', '8809793600310', '8809793600426', '8809793600396') then a11.ODR_QTY else 0 end)
   from    LGMJVDP.TB_STK_FT    a11
        join    LGMJVDP.TB_STORE_DM    a12
          on     (a11.STORECD = a12.STORECD)
          and a12.OPENDT < ('2024-06-11')
    where    (a11.DATECD between '2024-06-17' and '2024-07-31'
    ...
    group by    a11.STORECD, a11.GOODCD
     ...
     gopa2 as
 (select    a11.STORECD  STORECD,
        sum(case when a11.YMCD in ('202405','202406') then a11.SALDT_CNT else 0 end)  영업일수
    from    LGMJVDP.TB_SALDT_MS_AG    a11
        join    LGMJVDP.TB_STORE_DM    a12
          on     (a11.STORECD = a12.STORECD)
          and a12.OPENDT < ('2024-05-11')
    where    (a11.YMCD in ('202406')
     and a12.RGNCD between ('41') and ('64'))
    group by    a11.STORECD
    )
