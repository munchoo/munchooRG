...
/* CR191129059 총유형화상품수 */
CASE WHEN D.TARGET_BIZPL_YN IS NULL THEN NULL ELSE TO_CHAR((SELECT COUNT(1) FROM GSSCODS.TS_SL_AGRP_GOODS A WHERE A.ORIGIN_BIZPL_CD = B.ORIGIN_BIZPL_CD AND A.YYYYMM = '202204' AND A.SP = 'A')) END AS TOTAL_A_GOODS_CNT,
/* CR191129059 총 결품대상상품 수 -- 그냥 MAX를 하면 OPER_DT가 없는 날짜가 NULL이 되어 서브쿼리로 변경 */
(SELECT MAX(TARGET_GOODS_CNT) AS TARGET_GOODS_CNT FROM GSSCODS.TS_OP_OUTSTK A WHERE A.ORIGIN_BIZPL_CD = B.ORIGIN_BIZPL_CD AND A.BIZPL_CD = B.BIZPL_CD AND GOODS_SP ='A' AND SUBSTR(A.OPER_DT,1,6) = '202204' ) AS TARGET_GOODS_CNT,
/* CR191129059 총미취급상품수 */
CASE WHEN D.TARGET_BIZPL_YN IS NULL THEN NULL ELSE TO_CHAR((SELECT COUNT(1) FROM GSSCODS.TS_OP_MM_OUTSTK_SKU_NHDL A WHERE A.ORIGIN_BIZPL_CD = B.ORIGIN_BIZPL_CD AND A.BIZPL_CD = B.BIZPL_CD AND A.OPER_YYYYMM = '202204' AND A.GOODS_SP = 'A')) END AS TOTAL_NHDL_CNT,
/* CR191129059 총취급상품수 */
CASE WHEN D.TARGET_BIZPL_YN IS NULL THEN NULL ELSE TO_CHAR((SELECT COUNT(1) FROM GSSCODS.TS_OP_MM_OUTSTK_SKU_NHDL A WHERE A.ORIGIN_BIZPL_CD = B.ORIGIN_BIZPL_CD AND A.BIZPL_CD = B.BIZPL_CD AND A.OPER_YYYYMM = '202204' AND A.GOODS_SP = 'A' AND A.HDL_YN = 'Y')) END AS TOTAL_HDL_CNT,
